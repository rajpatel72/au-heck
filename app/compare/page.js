...
const d = parseFloat(discount || 0);
if (!usageVal || !rate) return 0;
const factor = noDiscountFields.includes(field) ? 1 : 1 - d / 100;
return usageVal * rate * factor;
};
...
// ✅ Helper to calculate final total for a section (manual/custom/retailer)
const calculateFinalTotal = (retailerKey) => {
let total = 0;

```
// Standard rows
fields.forEach((field) => {
  const usage = userInputs[field]?.usage || 0;
  const rate = userInputs[field]?.rate || 0;
  const discount = userInputs[field]?.discount || 0;

  if (retailerKey === "manual") {
    total += calcTotal(usage, rate, discount, field);
  } else if (data?.[retailerKey]) {
    const rateInDollars = parseFloat(data[retailerKey]?.[field]) || 0;
    const disc = parseFloat(data[retailerKey]?.["Discount"]) || 0;
    total += calcRetailerTotal(field, rateInDollars, disc, usage);
  }
});
...
customRows.forEach((row) => {
  if (retailerKey === "manual") {
    total += calcTotal(row.usage, row.rate, row.discount, row.field);
  } else {
    const rate = parseFloat(row[`${retailerKey.toLowerCase()}Rate`]) || 0;
    const disc = parseFloat(row[`${retailerKey.toLowerCase()}Discount`]) || 0;
    total += calcTotal(row.usage, rate, disc, row.field);
  }
});
...
return ( <main className="min-h-screen p-8"> <h1 className="text-3xl font-bold text-center mb-6">
⚡ Electricity Rate Comparison </h1>
        ...
        <tbody>
            ...
            return (
              <tr key={field} className="text-center hover:bg-gray-50">
                ...
                <td className="border p-2 font-semibold text-blue-700">
                  {calcTotal(usage, manualRate, manualDisc, field).toFixed(2)}
                </td>
          ...
          {customRows.map((row) => (
            <tr key={row.field} className="text-center bg-yellow-50">
              ...
              <td className="border p-2 font-semibold text-blue-700">
                {calcTotal(row.usage, row.rate, row.discount, row.field).toFixed(2)}
              </td>
          ...
          <tr className="font-bold bg-green-100 text-center">
            ...
            <td colSpan="3" className="border p-2 text-right pr-4">
              All Rows + Custom Rows
            </td>
            <td className="border p-2 text-blue-800">
              {calculateFinalTotal("manual")}
            </td>
            <td colSpan="2" className="border p-2 bg-gray-50">Origin</td>
            <td className="border p-2 bg-gray-50">
              {calculateFinalTotal("Origin")}
            </td>
            <td colSpan="2" className="border p-2 bg-gray-50">Nectr</td>
            <td className="border p-2 bg-gray-50">
              {calculateFinalTotal("Nectr")}
            </td>
            <td colSpan="2" className="border p-2 bg-gray-50">Momentum</td>
            <td className="border p-2 bg-gray-50">
              {calculateFinalTotal("Momentum")}
            </td>
            <td colSpan="2" className="border p-2 bg-gray-50">NBE</td>
            <td className="border p-2 bg-gray-50">
              {calculateFinalTotal("NBE")}
            </td>
          </tr>
        </tbody>
      </table>
