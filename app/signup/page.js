...
export default function SignupPage() {
  ...
  const logAction = async (checkboxLabel, isChecked, meta = {}) => {
    ...
    try {
      const res = await fetch('/api/log', {
        ...
        body: JSON.stringify({ name, checkboxLabel, isChecked, timestamp, meta }),
      });
  ...
  const renderMultiSiteFields = (baseLabel) => {
    return Array.from({ length: siteCount }, (_, i) => {
      const key = `${baseLabel} - Site ${i + 1}`;
      return (
        <label key={key} style={styles.checkboxLabel}>
          <input
            type="checkbox"
            checked={!!checkedItems[key]}
            onChange={() => handleSiteCheck(baseLabel, i + 1)}
                    ...
                    return (
                      <label key={key} style={styles.checkboxLabel}>
                        <input
                          type="checkbox"
                          checked={!!checkedItems[key]}
                          onChange={() => handleSiteCheck('Tariff screenshot (if applicable)', idx)}
                          style={{ marginRight: 10 }}
                        />
...
const styles = {
  main: { display: 'flex', justifyContent: 'center', minHeight: '100vh', background: '#eef2f3', fontFamily: 'Inter, Roboto, system-ui', padding: 24 },
